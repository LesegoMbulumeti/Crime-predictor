...
while($location = mysqli_fetch_array($location_report_info)){
  ...
  $sql = "SELECT * FROM case_info WHERE location = '$id'";
...
//print_r('location is' . $my_city_name);



$sql = '';
$sql = "SELECT * from case_info WHERE location = '$my_city_name'"; 
$resultss = mysqli_query($conn, $sql);


$crimes_per_loc = mysqli_num_rows($resultss);
...
$xxx = intval($crimes_per_loc);


$prediction = $prediction - $xxx;
//turn into %

$sql = "SELECT * FROM case_info";
$type_no_report_info = mysqli_query($conn, $sql);

$tot_no = mysqli_num_rows($type_no_report_info);

$prediction = ($prediction / $tot_no) * 100;


//for chart 2
//print_r($my_city_name);

$sql = "SELECT * FROM case_info WHERE location = '$my_city_name'";
$results = mysqli_query($conn, $sql);

//print_r(mysqli_fetch_assoc($results));
$type_no_array222 = Array();
$type_no_array2222 = Array();
while($type = mysqli_fetch_array($results)){

  //array_push($type_array, $type['crime_type']);

  $id = $type['crime_type_id'];
  $sql = "SELECT * FROM crime_type WHERE crime_type_id = '$id'";
  $type_no_report_info2 =  mysqli_query($conn, $sql);

  while($crime_name = mysqli_fetch_array($type_no_report_info2)) {
      $crime_name = $crime_name['crime_type'];
       array_push($type_no_array2, $crime_name);
}
    
}

$arr = array_count_values($type_no_array2);

array_push($type_no_array222, array_keys($arr));
array_push($type_no_array2222, array_values($arr));





//print_r($type_no_array222);
//echo nl2br("\n");
//print_r($type_no_array2222);
...
<div style="padding:0rem 2rem;" class="tile is-ancestor">
 ...
 <div class="tile is-parent is-4">
    <article class="tile is-child box">
    <p id="city_name" class="title">Crime at <?php echo $my_city_name ?></p>
    <div>
...
<section style= "margin: 10px, padding: 5px">
<div class="tile is-parent is-12">
    <article class="tile is-child box">
    <p class="title">Other crime news</p>
    <div>
 <iframe style="height:450px;width:100%" src="https://www.iol.co.za/news/crime-and-courts" title="Latest News"></iframe> 
</article>
</div>
</div>
</section>
...
<button onclick="printDiv('pdf01','Crime Prediction')" type="button" class="btn btn-warning">Generate Prediction Report</button>
...
function showChart2(){
  ...
  var type_array = [<?php echo json_encode($type_no_array222); ?>];
  var type_no_array = <?php echo json_encode($type_no_array2222); ?>;

console.log(type_array);
console.log(type_no_array);
...
type_array = type_array[0][0];
type_no_array = type_no_array[0];
console.log(type_array);
console.log(type_no_array);
